...
function setupGalaxy() {
    ...
    class Star {
        constructor() {
            ...
            const colors = ['#00f3ff', '#9d00ff', '#d946ef', '#ffffff'];
            this.color = colors[Math.floor(Math.random() * colors.length)];
        }
        update() {
            this.z -= 0.5; // Star moving speed
            if (this.z <= 0) {
                this.z = canvas.width;
                this.x = Math.random() * canvas.width;
                this.y = Math.random() * canvas.height;
        ...
        draw() {
            ...
            // Mouse parallax effect (shift perspective slightly)
            const moveX = (mouse.x - centerX) * 0.05;
            const moveY = (mouse.y - centerY) * 0.05;

            const scale = canvas.width / this.z;
            const x = (this.x - centerX) * scale + centerX + moveX;
            const y = (this.y - centerY) * scale + centerY + moveY;
...
function setupCursor() {
    if (window.innerWidth < 768) return; // Disable custom cursor on mobile touch screens
    ...
    function animateCursor() {
        // Smoothly interpolate outline position towards the mouse pointer
        outlinePos.x += (mouse.x - outlinePos.x) * 0.2;
        outlinePos.y += (mouse.y - outlinePos.y) * 0.2;
    ...
    const interactables = document.querySelectorAll('a, button, .btn, .project-card, input, textarea');
